...
TLSE *ret_imp_m_aux(TARVB *a, int N, TLSE *l){
  if(!a) return l;
  int i;
  i = a->nchaves - 1;
  while((i >= 0) && (a->chave[i] > N)){
    if(!a->folha) l = ret_imp_m_aux(a->filho[i+1], N, l);
    if((a->chave[i] != N) && (a->chave[i] % 2 == 1)) l = TLSE_insere(l, a->chave[i]);
    i--;
  } if(!a->folha) l = ret_imp_m_aux(a->filho[i+1], N, l);
  return l;
}

TLSE *ret_imp_m(TARVB *a, int N){
  return ret_imp_m_aux(a, N, NULL);
}
...
int main(int argc, char *argv[]){
  ...
  while(num != -1){
    ...
    // 2 1 2 7 8 9 10 12 57 18 11 27 39 99 97 131 -1
